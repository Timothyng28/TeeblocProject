...
# Load YOLO model
model = YOLO('/Users/timothy/projects/Teebloc/whitespace/runs/detect/train5/weights/best.pt')
...
def pseudo_label_images(image, pdf_file, page_num, model):
  ...
  # Class predictions
    ...
    # Save the image
    image_file_path = os.path.join(pseudo_label_path, "images")
    image_file_path = os.path.join(image_file_path, image_file_name)
    ...
    txt_file_path = os.path.join(pseudo_label_path, "annotations")
    ...
    with open(txt_file_path, 'w') as txt_file:
        for box, conf, cls in zip(boxes, confs, classes):
            ...
            font_thickness = 2  # Increase thickness for better visibility
            cv2.putText(vis_image, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, font_thickness)
